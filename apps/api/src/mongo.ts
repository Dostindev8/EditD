...
export async function connectMongo(): Promise<string> {
  ...
  try {
    ...
    log.log("Mongo available");
    return uri;
  } catch {
    if (process.env.NODE_ENV === "production" && process.env.ALLOW_MEMORY_MONGO !== "true") {
      throw new Error("MONGODB_URI is required in production (set ALLOW_MEMORY_MONGO=true only for demos)");
    }
    log.warn("Mongo unavailable — mongodb-memory-server (dev / demo)");
